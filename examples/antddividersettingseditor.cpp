...
AntdDividerSettingsEditor::AntdDividerSettingsEditor(QWidget *parent)
    : QWidget(parent),
      m_divider1(new QtAntdDivider()),
      m_divider2(new QtAntdDivider("Ant Design")),
      m_divider3(new QtAntdDivider("Right Text")),
      m_divider4(nullptr)
{
    ...
    m_textLineEdit = new QLineEdit("Ant Design");
    ...
    m_divider4 = new QtAntdDivider("Left Text");
    m_divider4->setTextAlignment(QtAntdDivider::Left);
    canvasLayout->addWidget(m_divider4);
    canvasLayout->addSpacing(20);
    
    canvasLayout->addWidget(new QLabel("Divider with Right Text:"));
    m_divider3->setTextAlignment(QtAntdDivider::Right);
...
AntdDividerSettingsEditor::~AntdDividerSettingsEditor()
{
    delete m_divider4;
}

void AntdDividerSettingsEditor::setupForm()
{
    updateWidget();
}

void AntdDividerSettingsEditor::updateWidget()
{
    // Update all dividers with current settings
    QtAntdDivider *dividers[] = {m_divider1, m_divider2, m_divider3, m_divider4};
    
    for (int i = 0; i < 4; ++i) {
        QtAntdDivider *divider = dividers[i];
        ...
        if (i > 0) {
            ...
            switch (m_textAlignmentComboBox->currentIndex()) {
                case 0: divider->setTextAlignment(QtAntdDivider::Left); break;
                case 1: divider->setTextAlignment(QtAntdDivider::Center); break;
                case 2: divider->setTextAlignment(QtAntdDivider::Right); break;
            }
        }
        
        switch (m_lineStyleComboBox->currentIndex()) {
            case 0: divider->setLineStyle(QtAntdDivider::SolidLine); break;
            case 1: divider->setLineStyle(QtAntdDivider::DashLine); break;
            case 2: divider->setLineStyle(QtAntdDivider::DotLine); break;
        }
...
void AntdDividerSettingsEditor::selectLineColor()
{
    QColorDialog dialog;
    ...
    if (dialog.exec()) {
        ...
        for (QtAntdDivider *divider : {m_divider1, m_divider2, m_divider3, m_divider4}) {
            divider->setLineColor(color);
        }
    }
}

void AntdDividerSettingsEditor::selectTextColor()
{
    QColorDialog dialog;
